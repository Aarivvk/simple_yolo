...
{
  ShortcutProjectionImpl(uint in_channels, uint out_channels, uint stride)
      : conv1(torch::nn::Conv2dOptions(in_channels, out_channels, 1).bias(false).stride(stride)),
...
{
  ResidualBlockImpl(uint in_channels, uint out_channels, uint stride)
      : conv1(torch::nn::Conv2dOptions(in_channels, out_channels, 3).stride(stride).padding(1)),
        batch_norm1(out_channels),
        conv2(torch::nn::Conv2dOptions(out_channels, out_channels, 3).stride(1).padding(1)),
...
{
  BottleneckResidualBlockImpl(uint in_channels, uint bottleneck_channels, uint out_channels, uint stride)
      : conv1(torch::nn::Conv2dOptions(in_channels, out_channels, 1).stride(1)),
        batch_norm1(out_channels),
...
{
  ResNetBaseImpl(
      std::vector<uint> n_blocks,
      std::vector<uint> n_channels,
      std::vector<uint> bottlenecks,
      uint img_channels = 3,
      uint first_kernel_size = 7)
      : conv1(torch::nn::Conv2dOptions(img_channels, n_channels[0], first_kernel_size)
                  .stride(2)
                  .padding(std::floor(first_kernel_size / 2))),
