...
cv::Mat& mark_target(cv::Mat& image, const VOC::Target& target, size_t s=7)
{
...
cv:
  ...
  cv::putText(
      image, cell_name, cv::Point(0, 20), cv::FONT_HERSHEY_DUPLEX, 1, cv::Scalar(0, 255, 0), 2, false);
  return image;
}

cv::Mat& mark_cells(cv::Mat& image, const size_t s=7)
{
  for (int i = 0; i < s; i++)
  {
    for (int j = 0; j < s; j++)
